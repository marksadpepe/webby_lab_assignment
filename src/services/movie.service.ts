...
class MovieService {
  async importMovies(
    file?: any,
  ): Promise<GenericResponse<CreateMovieResponse>> {
    if (!file) {
    ...
    const existing = await MovieModel.findAll({
    ...
    return await sequelize.transaction(async transaction => {
      ...
      for (const movieItem of parsed) {
        ...
        const movieHasActorsList = actorModels.map(a => ({
          movieId: movie.id,
          actorId: a.id,
        }));
    ...
    return await sequelize.transaction(async transaction => {
      ...
      if (existingMovies.length) {
        throw ApiException.ConflictException(
          'One of these movies already exist',
        );
      }
      ...
      for (const movie of movieList) {
        ...
        const movieEntity = await MovieModel.create(
          { title, year, format },
          { transaction },
        );
        ...
        await MovieActorModel.bulkCreate(movieHasActorsRelations, {
          transaction,
        });
  ...
  ): Promise<GenericResponse<GetMovieResponse[]>> {
    const { title, actorName } = params;

    const movieWhereOptions: WhereOptions | undefined = title
      ? { title }
      : undefined;
    const actorWhereOptions: WhereOptions | undefined = actorName
      ? { fullName: actorName }
      : undefined;
